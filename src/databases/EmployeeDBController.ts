...
export class EmployeeDBController {
  ...
  async CreateEmployee(
    ...
    try {
      ...
      // Crea el empleado
      const employeeToAdd: mongoose.Document<IEmployee> =
      ...
      await employeeToAdd.save().catch(() => {
        return false;
      });
      return true;
    } catch (err) {
  ...
  async UpdateEmployee(
    ...
    try {
      const empleadoEnDb = await this.CollectionModel.exists({
        _id: Empleado._id,
      });
      ...
      if (!password) {
        ...
        const resultadoUpdate = await this.CollectionModel.updateOne(
          { _id: Empleado._id },
          { $set: updatedEmpleado }
        );
