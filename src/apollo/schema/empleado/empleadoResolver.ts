...
export const empleadoResolver = async (
  parent: any,
  args: EmpleadoFind,
  context: any,
  info: any
  ...
  if (args.find._id) {
    let e = await db.EmployeeDBController.CollectionModel.findOne({
      _id: args.find._id,
    }).exec();
...
export const empleadosResolver = async (
  parent: any,
  args: EmpleadosFind,
  context: any,
  info: any
  ...
  if (args.find?.isLibre !== null && args.find?.isLibre !== undefined) {
    ...
    const empleadosMap = new Map<
      string,
      IEmployee & { _id: mongoose.Types.ObjectId }
    >();
    ...
    for (let index = 0; index < tpvs.length; index++) {
      ...
      if (args.find.isLibre) {
        ...
        const empleado = empleados.find(
          (emp) => emp._id.valueOf() === tpv.enUsoPor._id.valueOf()
        );
        if (!empleado) {
          continue;
        }
...
export const addEmpleadoResolver = async (
  root: any,
  args: any,
  context: any
) => {
  ...
  try {
    ...
    const empleadoAñadido = await db.EmployeeDBController.CreateEmployee(
      Empleado,
      args.empleadoInput.password
    );
...
export const deleteEmpleadoResolver = async (
  root: any,
  args: any,
  context: any
) => {
  ...
  try {
    ...
    const res = await db.EmployeeDBController.CollectionModel.deleteOne({
      _id: args._id,
    });
...
export const updateEmpleadoResolver = async (
  root: any,
  args: any,
  context: any
) => {
  ...
  try {
    ...
    const actualizadoCorrectamente =
      await db.EmployeeDBController.UpdateEmployee(
        updatedEmpleado,
        args.empleadoInput.password
      );
