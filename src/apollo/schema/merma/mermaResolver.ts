...
import {
  ...
  MermaInput,
  MermasFind,
  NuevoProductoMermado,
} from "../../../types/types";

export const mermaResolver = async (
  parent: any,
  args: MermaFind,
  context: any,
  info: any
  ...
  if (args.find._id) {
    const merma = await db.MermaDBController.CollectionModel.findOne({
      _id: args.find._id,
    }).exec();
...
export const mermasResolver = async (
  parent: any,
  args: MermasFind,
  context: any,
  info: any
  ...
  if (args.find?.query) {
    ...
    const mermas = await db.MermaDBController.CollectionModel.find({
      ...
      $and:
        args.find.fechaInicial && args.find.fechaFinal
          ? [
              {
                createdAt: {
...
export const addMermaResolver = async (
  root: any,
  args: { merma: MermaInput },
  context: any
) => {
  // Check de autenticidad para aceptar peticiones válidas. Descomentar en producción
  // if (!context.user) { throw new UserInputError('Usuario sin autenticar'); }
  try {
    ...
    const newMerma: mongoose.Document<IMerma> =
      new db.MermaDBController.CollectionModel(merma);
...
export const deleteMermaResolver = async (
  root: any,
  args: { _id: string },
  context: any
) => {
...
export const updateMermaResolver = async (
  root: any,
  args: { _id: string; merma: MermaInput },
  context: any
) => {
  // Check de autenticidad para aceptar peticiones válidas. Descomentar en producción
  // if (!context.user) { throw new UserInputError('Usuario sin autenticar'); }
  try {
    ...
    const mermaVieja = await db.MermaDBController.CollectionModel.findOne({
      _id: args._id,
    });
    if (mermaVieja === null) {
      return {
    ...
    const newMerma: mongoose.Document<IMerma> =
      new db.MermaDBController.CollectionModel(updatedMerma);
    const mermaGuardada = await newMerma.save();
...
const CalcularMermaValues = async (
  productosMermados: NuevoProductoMermado[]
): Promise<{ costeProductos: number; ventas: number; beneficio: number }> => {
  const db = Database.Instance();
  ...
  let costes: { costeProductos: number; ventas: number; beneficio: number } = {
    costeProductos: 0,
    ventas: 0,
    beneficio: 0,
  };
  for (let index = 0; index < productosMermados.length; index++) {
    ...
    const precioCompraIva =
      producto.precioCompra + producto.precioCompra * (producto.iva / 100);
...
const GetProductosMermados = async (
  productosMermados: NuevoProductoMermado[]
): Promise<IProductoMermado[]> => {
  const db = Database.Instance();
  const prodMap = new Map<string, IProduct>();
...
const DeleteMerma = async (
  _id: string
): Promise<{ message: string; successful: boolean }> => {
  try {
    const db = Database.Instance();
    ...
    if (!merma) {
      return {
        message:
          "No se ha encontrado la merma que se quiere eliminar o modificar",
        successful: false,
      };
    }

    const actualizadoCorrectamente = await ActualizarCantidadProductos(
      merma,
      true
    );
    ...
    const deletedMerm = await db.MermaDBController.CollectionModel.deleteOne({
      _id: _id,
    });
...
const ActualizarCantidadProductos = async (
  merma: IMerma | (IMerma & { _id: mongoose.Types.ObjectId }),
  isDeleting?: boolean
): Promise<boolean> => {
  try {
    const db = Database.Instance();
    ...
    for (let index = 0; index < merma.productos.length; index++) {
      ...
      const res = await db.ProductDBController.CollectionModel.updateOne(
        { _id: productoMermado._id },
        {
          $inc: {
            cantidad: isDeleting
