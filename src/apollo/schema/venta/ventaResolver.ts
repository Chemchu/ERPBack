...
export const ventaResolver = async (
  parent: any,
  args: VentaFind,
  context: any,
  info: any
  ...
  if (args._id) {
    const venta = await db.VentasDBController.CollectionModel.findOne({
      _id: args._id,
    }).exec();
...
export const ventasResolver = async (
  parent: any,
  args: VentasFind,
  context: any,
  info: any
  ...
  if (args.find?.createdAt && args.find.tpv) {
    ...
    if (args.order) {
      order = args.order as mongoose.SortOrder;
    }
    const ventas = await db.VentasDBController.CollectionModel.find({
      tpv: args.find.tpv,
...
export const addVentaResolver = async (root: any, args: any, context: any) => {
  ...
  try {
    ...
    const saleToAdd: mongoose.Document<ISale> =
      new db.VentasDBController.CollectionModel(ventaFixed);
    const res: any = await saleToAdd.save();

    let isUpdatingCorrectly = true;
    // Actualizar la cantidad de productos
    args.fields.productos.forEach(async (p: ISoldProduct) => {
      const err = await db.ProductDBController.CollectionModel.findOneAndUpdate(
        { _id: p._id },
    ...
    if (!isUpdatingCorrectly) {
      return {
        message:
          "Venta añadida pero las cantidades no han sido actualizadas correctamente",
        successful: true,
      };
    }
...
export const deleteVentaResolver = async (
  root: any,
  args: any,
  context: any
) => {
...
export const updateVentaResolver = async (
  root: any,
  args: any,
  context: any
) => {
  ...
  const db = Database.Instance();
  const ventaOriginal = await db.VentasDBController.CollectionModel.findOne({
    _id: args._id,
  });
  if (!ventaOriginal) {
  ...
  const resultadoUpdate = await db.VentasDBController.CollectionModel.updateOne(
    { _id: args._id },
    { $set: venta }
  );
  const updatedSale: any = await db.VentasDBController.CollectionModel.findOne({
    _id: args._id,
  });
...
const FixVentaConsistency = async (venta: any): Promise<ISale> => {
  const db = Database.Instance();
  const numVentas =
    await db.VentasDBController.CollectionModel.countDocuments();
  const currentYear = new Date().getFullYear();
  const nFactura = `${currentYear}/${numVentas + 1}`;

  try {
    const [
      productosVendidosFixed,
      precioVentaTotalProductos,
      precioVentaTotalSinDtoProductos,
    ] = FixProductsInconsistency(venta.productos);

    if (
      productosVendidosFixed.length <= 0 ||
      venta.productos.length != productosVendidosFixed.length
    ) {
      return CreateUncheckedSale(venta, nFactura);
    }

    let precioVentaTotal =
      (precioVentaTotalProductos - venta.descuentoEfectivo) *
      (1 - venta.descuentoPorcentaje / 100);
    const cambio =
      venta.dineroEntregadoEfectivo +
...
const FixProductsInconsistency = (
  productos: ISoldProduct[]
): [ISoldProduct[], number, number] => {
  try {
    ...
    for (let index = 0; index < productos.length; index++) {
      ...
      try {
        ...
        if (producto.margen <= 0 || !producto.margen) {
          const iva = producto.iva || 10;
          const precioConIva =
            producto.precioCompra + producto.precioCompra * (iva / 100);
