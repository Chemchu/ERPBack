...
export const proveedoresResolver = async (
  parent: any,
  args: { find: any; limit: number },
  context: any,
  info: any
) => {
...
export const addProveedorResolver = async (
  root: any,
  args: { fields: IProveedor },
  context: any
) => {
  const db = Database.Instance();

  const prov = await db.ProveedorDBController.CollectionModel.exists({
  ...
  const cif = await db.ProveedorDBController.CollectionModel.exists({
  ...
  const proveedorContacto: IProveedorContacto = {
    ...
    email: String(args.fields.contacto?.email).toLowerCase() || "",
  };

  const updatedProv: mongoose.Document<IProveedor> =
    new db.ProveedorDBController.CollectionModel({
      nombre: args.fields.nombre,
      cif: args.fields.cif,
...
export const deleteProveedorResolver = async (
  root: any,
  args: { _id: string },
  context: any
) => {
  ...
  const deletedProd = await db.ProveedorDBController.CollectionModel.deleteOne({
    _id: args._id,
  });
...
export const updateProveedorResolver = async (
  root: any,
  args: { _id: string; proveedor: IProveedor },
  context: any
) => {
  // Check de autenticidad para aceptar peticiones válidas. Descomentar en producción
  // if (!context.user) { throw new UserInputError('Usuario sin autenticar'); }
  ...
  const resultadoUpdate =
    await db.ProveedorDBController.CollectionModel.updateOne(
      { _id: args._id },
      { $set: updatedProv }
    );
