...
export const tpvResolver = async (
  parent: any,
  args: TPVFind,
  context: any,
  info: any
  ...
  if (!args.find.empleadoId) {
    if (args.find._id) {
      return await db.TPVDBController.CollectionModel.findOne({
        _id: args.find._id,
      }).exec();
    }
...
export const tpvsResolver = async (
  parent: any,
  args: TPVsFind,
  context: any,
  info: any
...
export const transferirTpvResolver = async (
  root: any,
  args: { idEmpleadoDestinatario: string; idTPV: string },
  context: any
  ...
  if (tpv.libre)
    return {
      message: "No se puede transferir una TPV que no está en uso",
      token: null,
      successful: false,
    };

  // Comprobar empleado remitente
  const empleadoRemitente =
    await db.EmployeeDBController.CollectionModel.findOne({
      _id: tpv.enUsoPor._id,
    });
  if (!empleadoRemitente) {
    return {
      message: "Empleado remitente no encontrado",
      token: null,
      successful: false,
    };
  }
  ...
  const empleadoDestinatario =
    await db.EmployeeDBController.CollectionModel.findOne({
      _id: args.idEmpleadoDestinatario,
    });
  if (!empleadoDestinatario) {
    return {
      message: "Empleado destinatario no encontrado",
      token: null,
      successful: false,
    };
  }
  ...
  for (let index = 0; index < tpvs.length; index++) {
    ...
    if (tpv.enUsoPor._id === args.idEmpleadoDestinatario) {
      return {
        message: "El empleado destinatario está usando otra TPV",
        successful: false,
        token: null,
      };
    }
  ...
  if (res.modifiedCount <= 0) {
    return {
      message: "No se ha podido transferir la TPV",
      successful: false,
      token: null,
    };
  }
  ...
  return {
    message: "TPV transferida correctamente",
    successful: true,
    token: `Bearer ${token}`,
  };
};

export const ocupyTpvResolver = async (
  root: any,
  args: { idEmpleado: string; idTPV: string; cajaInicial: number },
  context: any
) => {
  const db = Database.Instance();
...
export const freeTpvResolver = async (
  root: any,
  args: { idEmpleado: string; idTPV: string },
  context: any
) => {
  const db = Database.Instance();
  ...
  if (!empleado) {
    throw new UserInputError("Empleado no encontrado");
  }
  ...
  if (tpv.libre)
    throw new UserInputError("TPV actualmente ocupada por otro empleado");
