...
import {
  CierreTPVInput,
  CierreTPVUpdateInput,
  JWTPayload,
} from "../../../types/types";
import { ICierreTPV } from "../../../types/TPV";
import mongoose from "mongoose";

export const cierreTpvResolver = async (
  parent: any,
  args: any,
  context: any,
  info: any
...
export const cierreTpvsResolver = async (
  parent: any,
  args: any,
  context: any,
  info: any
  ...
  try {
    ...
    if (args.find.query) {
      ...
      let tpv;
      if (isQueryValidId) {
        const cierre = await db.CierreTPVDBController.CollectionModel.findById(
          args.find.query
        );
        if (cierre !== null) {
          return [cierre];
        ...
        tpv = await db.TPVDBController.CollectionModel.findById(
          args.find.query
        );
      }
...
export const addCierreTpvResolver = async (
  root: any,
  args: { cierre: CierreTPVInput },
  context: any
) => {
  try {
    ...
    const empleadoCerrando =
      await db.EmployeeDBController.CollectionModel.findById(
        args.cierre.empleadoCerrandoId
      );
    if (!empleadoCerrando) {
      return {
    ...
    const tpvUpdated = await db.TPVDBController.CollectionModel.updateOne(
      { _id: tpv._id },
      { libre: true, abiertoPor: null, enUsoPor: null, fechaApertura: null }
    );
...
export const deleteCierreTpvResolver = async (
  root: any,
  args: any,
  context: any
) => {
  ...
  const deletedProd = await db.CierreTPVDBController.CollectionModel.deleteOne({
    _id: args._id,
  });
...
export const updateCierreTpvResolver = async (
  root: any,
  args: { cierre: CierreTPVUpdateInput },
  context: any
) => {
  // Check de autenticidad para aceptar peticiones válidas. Descomentar en producción
  // if (!context.user) { throw new UserInputError('Usuario sin autenticar'); }
