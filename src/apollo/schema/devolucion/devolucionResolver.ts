...
export const devolucionResolver = async (
  parent: any,
  args: VentaFind,
  context: any,
  info: any
  ...
  if (args._id) {
    const venta = await db.DevolucionDBController.CollectionModel.findOne({
      _id: args._id,
    }).exec();
...
export const devolucionesResolver = async (
  parent: any,
  args: DevolucionFind,
  context: any,
  info: any
...
export const addDevolucionResolver = async (
  root: any,
  args: any,
  context: any
) => {
  ...
  try {
    ...
    const trabajador = await db.EmployeeDBController.CollectionModel.findOne({
...
export const deleteDevolucionResolver = async (
  root: any,
  args: any,
  context: any
) => {
...
export const updateDevolucionResolver = async (
  root: any,
  args: any,
  context: any
) => {
...
const ActualizarStock = async (db: Database, fields: any) => {
  fields.productosDevueltos.forEach(async (p: IReturnProduct) => {
    await db.ProductDBController.CollectionModel.findOneAndUpdate(
      { _id: p._id },
      { $inc: { cantidad: +p.cantidadDevuelta } }
    );
  });
};
...
const ActualizarVenta = async (
  ...
  if (numProdEnVenta === numProdDevuelto) {
    await db.VentasDBController.CollectionModel.deleteOne({
      _id: fields.ventaId,
    });
  } else {
    ...
    let updatedVenta = {
      ...
      cambio: Number(
        (cambio + (ventaOriginal.precioVentaTotal - precioVentaTotal)).toFixed(
          2
        )
      ),
    };

    const vUpdated = await db.VentasDBController.CollectionModel.updateOne(
      { _id: fields.ventaId },
      updatedVenta
    );
  }
